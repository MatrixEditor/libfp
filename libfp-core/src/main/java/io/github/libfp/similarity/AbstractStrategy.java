package io.github.libfp.similarity; //@date 09.11.2023
        ...
        implements IStrategy<S>
{
    ...
    private final Map<Integer, ISimilarityStrategy<?>> strategies =
            new HashMap<>();

    private final Map<Integer, List<IStep<?, ?>>> steps =
            new HashMap<>();

    private final Set<ProfilePolicy<?>> policies = new HashSet<>();

    @Override
    public @NotNull <C> Collection<ProfilePolicy<C>> getPolicies(Class<? extends C> type)
    {
    ...
    {
        //noinspection unchecked
        return (ISimilarityStrategy<T>) this.strategies.get(TypeNames.hash(type));
    }
    ...
    @NotNull Collection<IStep<T, M>> getFeatureExtractors(@NotNull Class<M> type)
    {
        final int hash = TypeNames.hash(type);
    ...
    {
        final int hash = TypeNames.hash(type);
        steps.computeIfAbsent(hash, k -> new LinkedList<>())
             .add(extractor);
        return self();
    }
     ...
     * @return The current instance of the concrete strategy class.
     */
    public <M extends ManagedProfile> @NotNull S remove(@NotNull Class<M> type)
    {
        final int hash = TypeNames.hash(type);
        steps.remove(hash);
        return self();
     ...
     */
    @Override
    public <T extends IStrategy<T>> @NotNull S with(@NotNull IStrategy<T> other)
    {
    ...
    {
        StringJoiner strategies = new StringJoiner(",", "strategies=", "");
